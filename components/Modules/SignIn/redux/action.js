...
export const signUp = (data, callBack, router) => (dispatch) => {
	...
	Service.post(API.signUp, data)
		.then((res) => {
			...
			if (callBack) {
        let msg =
					res?.data?.data?.user_type === "ADMIN"
						? "Admin created successfully"
						: data?.is_login
						? " Logged in success!"
						: "Sign up has been completed. Please login now.";
				callBack(true, msg);
			}
		})
		.catch((err) => {
...
export const getAuthToken = (callBack) => (dispatch) => {
	Service.get(API.getAuthToken + "?userId=" + localStorage.getItem("userId"), {
