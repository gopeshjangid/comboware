...
export const saveSkills = (data, callback) => (dispatch) => {
	...
	Service.post(API.saveSkills, data)
		.then((res) => {
			const msg =
				data?.type === "SKILL"
					? "Saved skill successfully."
					: "Saved level successfully";
			callback(true, msg);
			dispatch(getSkillLevels(data.type, () => {}));
			dispatch(
				saveResourceData({ message: "Resource info updated successfully." })
			);
		})
		.catch((err) => {
			console.log("err--", err);
...
export const changeSkillLevelStatus = (data, callback) => (dispatch) => {
	...
	Service.put(API.changeSkillLevelStatus, data)
		.then((res) => {
			let flag = data.status ? "Enabled" : "Disabled";
			const msg =
				data?.type === "SKILL"
					? flag + " skill successfully."
					: flag + " level successfully";
			callback(true, msg);
			dispatch(getSkillLevels(data.type, () => {}));
		})
		.catch((err) => {
