...
export const updateProfile = (data, callback) => (dispatch) => {
	Service.put(API.updateProfile, data)
		.then((res) => {
			if (callback) {
...
export const getProfile = (id) => (dispatch) => {
	Service.get(API.getProfile + "?userId=" + id)
		.then(({ data }) => {
			dispatch(
				saveRequest({
			...
			dispatch(
				saveProfile({ profile: data?.data?.user, skills: data?.data?.skills })
			);
		})
		.catch((err) => {
			console.log("err--", err);
...
export const updateSystemInfo = (data) => (dispatch) => {
	...
	Service.put(API.updateSystemInfo + data?.query, data?.body)
		.then((res) => {
...
export const logoutUser = (data) => (dispatch) => {
	...
	dispatch(
		saveRequest({ domain: { id: "", name: "" }, project: { id: "", name: "" } })
	);
};

export const usersList = (qStr) => (dispatch) => {
	dispatch(requestInit({ loading: true }));
	Service.get(API.getAllUsers + qStr)
		.then((res) => {
