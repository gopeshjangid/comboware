...
const CustomerManagement = ({ getAllUserList, getProfile }) => {
	...
	const getColumns = () => {
		return [
			...
			{
				field: "is_profile_setup",
				header: "Profile Status",
				renderCell: (row) => {
					return (
						<Chip
							style="info"
							type={!row?.is_profile_setup ? "outlined" : "primary"}
							label={row?.is_profile_setup ? "COMPLETED" : "INCOMPLETE"}
							color="primary"
						/>
					);
				},
			...
			// },
			{
				...
				renderCell: (row) => {
					return (
						<Chip
							style="info"
							type={!row?.status ? "outlined" : "primary"}
						...
						tabs={[
							{
								...
								panel: (
									<CustomTable
										columns={getColumns()}
										data={reduxState.engineers_list}
									/>
								),
							},
							{
					...
					{userDetail && (
						<List component="nav" aria-label="main mailbox folders">
							<ListItem button>
								<ListItemText
									primary={"Name:"}
									secondary={`${userDetail.first_name ||
							...
							<ListItem button>
								<ListItemText
									primary={"UserName:"}
									secondary={userDetail.user_name || ""}
								/>
							</ListItem>
							<ListItem button>
								<ListItemText
