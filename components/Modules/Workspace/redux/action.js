...
export const serverRequest = (data, callback) => (dispatch) => {
	if (callback) {
		callback(-1, "Please wait...");
	}
	Service.post(API.createServerRequest, data)
		.then((res) => {
			if (callback) {
				callback(true, "Server request has been created successfully.");
			}
			dispatch(
				saveRequest({
					data: { server: { ...res?.data?.data } },
				})
			);
		})
		.catch((err) => {
...
export const getWorkSpaceDetails = (id, callback) => (dispatch) => {
	//dispatch(requestStart({ message: "Fetching workspace details..." }));
	Service.get(API.getServerDetails + "?userId=" + id)
		.then((res) => {
			//	console.log("res", res?.response);
