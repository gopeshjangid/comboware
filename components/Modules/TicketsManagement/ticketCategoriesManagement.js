...
import { getAllTickets, getAllCategories, saveCategory, saveSubcategory, enableDisableCatSubCat } from './redux/action';
...
const TicketCategoriesManagement = ({ getAllCategories, saveCategory, saveSubcategory, enableDisableCatSubCat }) => {
  ...
  const submitHandler = async (e, type) => {
    ...
    if (type === 'category') {
      if (formStatus === 'edit') {
        await saveCategory(categoryFromData);
      } else {
        await saveCategory({
      ...
      if (selectedCategory) {
        if (formStatus === 'edit') {
          await saveSubcategory(subCategoryFromData);
        } else {
        ...
        <GridItem xs={12} sm={6}>
          <FieldSet padding={30} title='Ticket Categories'>
            <GridContainer spacing={2} mb={3} style={{ marginBottom: 30 }}>
              ...
              <GridItem justify='flex-end' xs={12} style={{ textAlign: 'right' }}>
...
export default connect(
  ...
  { getAllTickets, getAllCategories, saveCategory, saveSubcategory, enableDisableCatSubCat }
