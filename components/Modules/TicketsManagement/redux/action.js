...
export const addNewActivity = (data, callBack) => (dispatch, getState) => {
  ...
  Service.post(ticketConstants.API.addActivities + data?.query, jsonBody, headers)
    .then((res) => {
      let state = getState();
      ...
      dispatch(
        saveTicket({
          ticketDetails: {
            ...
            activities: [...state?.ticket?.ticketDetails?.activities, res?.data?.data[0]]
          },
...
export const getTicketDetails = (ticketId, userId) => (dispatch) => {
  ...
  Service.get(
    ticketConstants.API.getTicketDetails +
      `?ticketNumber=${ticketId}&userId=${userId}&userType=${localStorage.getItem('userType')}`
  )
    .then((res) => {
      console.log('res', res?.response);
...
export const getAllCategories = () => (dispatch) => {
  ...
  Service.get(`${ticketConstants.API.getAllCategories}`)
    .then((res) => {
      dispatch({ type: ticketConstants.GET_ALL_CATEGORIES, payload: res?.data?.data });
    })
    .catch((err) => {
      console.log('err--', err);
...
export const enableDisableCatSubCat = (data) => (dispatch) => {
  dispatch(start({ message: 'Creating ticket...' }));
