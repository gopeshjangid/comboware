...
import { getWorkSpaceDetails } from '../Workspace/redux/action';
import { addNewActivity, createTicket, getCategories, getSubCategories } from './redux/action';
...
function newTicket({ createTicket, addNewActivity, getCategories, getSubCategories }) {
  ...
  const activitySubmit = (ticketNumber) => {
    ...
    if (image) {
      addNewActivity(
        { type: 'image', query: '?ticketNumber=' + ticketId + '&userId=' + userId, body: image },
        hideNotification
      );
    }
    if (note) {
      addNewActivity(
        { query: '?ticketNumber=' + ticketId + '&userId=' + userId, body: { activities: [note] } },
        hideNotification
      );
      setNote(null);
    }
    ...
    <Wrapper>
      ...
      <FieldSet title='New Ticket'>
        <GridContainer spacing={1}>
          <GridItem xs={12}>
            <fieldset className={classes.boxModal} borderColor='#e7e9f0' border={0.5}>
          ...
          <GridItem xs={10} align='right'>
            <Button type='error' className={classes.submit} onClick={() => router.push('/ticket')}>
              Cancel
            </Button>
          </GridItem>
