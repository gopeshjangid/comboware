...
function Settings({ saveCluster, settings }) {
	...
	const hostSubmitHandler = (e) => {
		...
		e.preventDefault();
		if (
			Object.values(cluster).every((val) => val !== "") &&
			...
			setMessage({
				type: "error",
				content: "Please fill all the valid details.",
			});
		}
	};
							...
							<GridContainer spacing={2}>
								...
								<GridItem className={classes.gridRow} xs={3}>
									<TextField
										...
										helperText={
											error?.cluster_ip
												? "Cluster ip is not valid."
												: "ex.: https://10.89.90.12"
										}
									/>
								</GridItem>
								<GridItem className={classes.gridRow} xs={6}>
									<TextField
										name="cluster_admin_url"
										fullWidth
										label="Cluster Admin URL"
										value={cluster?.cluster_admin_url}
										onChange={hostChangeHandler}
										type="text"
										inputProps={{ min: 0 }}
										onKeyPress={blurHandler}
										error={error?.cluster_admin_url}
										helperText={
											error?.cluster_admin_url
												? "Cluster ip is not valid."
												: "ex. https://192.168.60.2:5800"
										}
									/>
								</GridItem>
								<GridItem className={classes.gridRow} xs={6}>
